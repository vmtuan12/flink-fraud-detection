...
ds_transaction = env.from_source(transaction_source, WatermarkStrategy.for_bounded_out_of_orderness(Duration.of_seconds(10)), "Transaction Source")
ds_rule = env.from_source(rule_source, WatermarkStrategy.no_watermarks(), "Rule Source")

key_transaction_ds = ds_transaction.key_by(lambda record: record["user_id"], key_type=Types.INT())
...
# detection_ds = key_transaction_ds.connect(rule_broadcast_stream).process(PatternBroadcastProcessFunction())